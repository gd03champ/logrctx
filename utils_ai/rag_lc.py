...
def rag_chat_init(dir_path, filename):
    ...
    # Vector store things
    console.print(Panel.fit("[bold cyan]Creating vector store..."))
...
Feel free to use markdown format wherever possible for visually appealing output.
...
<|assistant|>
    ...
    console.print(Panel.fit("[bold cyan]Retrieving context..."))
    ...
    while True:
        ...
        generating_status = console.status("[bold]Generating response...")
        #generating_status.start()

        response = retrieval_chain.invoke({"input": query})

        #generating_status.stop()
        print("\n")
        console.print(Panel.fit("[bold cyan]Rendering markdown..."))
        console.print(Panel.fit(Text.from_markup(f"[bold]Answer:[/bold] {response['answer']}")))
